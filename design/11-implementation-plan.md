...
- Generic `Graph` + Dijkstra + `GraphPath`; Tier-2 A* as a resumable `Search`; `HeuristicStrategy`
  (admissible + running-average); recalc loop; cancellation & limits; `PathString` builder;
  `OdysseyApi` impl.
...
- `MinecraftModeType`, `MinecraftMode`, `MinecraftAgent`/`OdysseyPlayer`, `Direction`; `OdysseyBlock`
  /`Chunk`/`World`, `ChunkProvider` (LRU + staleness + read-ahead + de-dup), `PlatformApi`/`Scheduler`
  contracts, `MinecraftTraversalState`; all real modes (Walk/Jump/Swim/Fly/Mine/Fall/Boat/Horse).
...
- **Verify (real server):** boot a Paper/Folia test server; `/nav` to a waypoint and walk the
  ...
  cancel on logout; portal discovery creates tunnels; `/odyssey reload` respects mutable/immutable;
  tune `read_ahead_margin` and cache size against real search latency.

## Phase 7 — remaining backends + `sponge-16` + `sponge-plugin`  (`05`,`06`,`07`)
...
- **Verify:** on a server with each target plugin, confirm destinations/tunnels/navigator appear and
  route correctly (e.g. `/nav home` uses the Essentials teleport tunnel; `guide` NPC walks the path).
