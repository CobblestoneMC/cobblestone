 ...
 *
 * <p>This is the linchpin of Cobblestone's cooperative scheduling: a search consumes {@link
 * Immediate} results in a tight synchronous loop (cache hits) and only registers a continuation —
 * parking its worker thread — when it encounters a {@link Pending} value (a cache miss). Modelled
 * as a sealed sum type so callers can switch exhaustively.
 *
