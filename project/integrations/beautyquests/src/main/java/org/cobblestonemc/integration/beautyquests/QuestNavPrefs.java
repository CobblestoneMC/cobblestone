...
/**
 * Resolves CobblestoneBeautyQuests' per-quest navigation preferences from the plugin config:
 * whether a quest auto-navigates, and the {@link NavigatorSettings} (navigator id + trail
 * particles/colors) to guide it with. Each field falls back from a per-quest override ({@code
 * quests.<id>.<field>}) to the global default, so an admin styles everything once and tweaks
 * individual quests as needed. Quests are keyed by their numeric BeautyQuests id, passed here as a
 * string.
 *
 * <p>The config is read live (through the plugin's {@link FileConfiguration}), so a reload takes
 * effect on the next quest stage change without a restart.
 */
final class QuestNavPrefs {
  ...
  /**
   * Whether a trip should be auto-started when a player advances to a locatable stage of {@code
   * questId}.
   */
