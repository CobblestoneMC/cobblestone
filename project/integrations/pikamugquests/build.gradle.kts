// cobblestone-pikamugquests — connects PikaMug's Quests to Cobblestone. When a quest's compass target updates
// for a player, it starts (or replaces) a guided Cobblestone trip to that objective; it also surfaces
// each active quest's current locatable objective as a `/navigate pikamugquests quest <name>`
// destination. A thin, un-shaded jar: it compiles against Cobblestone's published API and the Quests API,
