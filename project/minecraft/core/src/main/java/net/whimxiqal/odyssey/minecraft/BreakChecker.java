 ...
 * it emits the mining edge optimistically and attaches the future to the {@link
 * net.whimxiqal.odyssey.Movement}, so an already-completed future (a synchronous block-type rule)
 * never parks the search. Distinct from {@link MinecraftAgent#canBreak} (the coarse, synchronous
 * per-agent gate); both must allow.
 *
...
public interface BreakChecker<A extends MinecraftAgent> {
  ...
  CompletableFuture<Boolean> breakable(
      A agent, Cell cell, MinecraftWorld world, MinecraftBlock block);
}
