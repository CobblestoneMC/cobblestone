...
/**
 * Remembers which chunks already exist on disk, per world, so Cobblestone can tell "load this
 * chunk" apart from "generate this chunk" before it asks for either.
 *
 ...
 * generation event and by every chunk Cobblestone successfully reads. Chunks that exist only in
 * memory and have never been saved are not in the region files, but they are by definition loaded,
 * and a loaded chunk never reaches this index — it is served before the question is asked.
 */
final class ChunkExistenceIndex {
  ...
  ChunkExistenceIndex(SpongeScheduler scheduler, CobblestoneLogger logger) {
  ...
  private void scan(ResourceKey key, ServerWorld world, WorldIndex index) {
    ...
    try (Stream<Vector3i> positions = world.chunkPositions()) {
      ...
      index.failed();
      logger.error(
          "Could not index the existing chunks of world '{}'. Cobblestone will not load unloaded"
