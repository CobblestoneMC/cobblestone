...
import net.whimxiqal.odyssey.plugin.config.Codec;
import net.whimxiqal.odyssey.plugin.config.ConfigKey;
 ...
 *
 * <p>Sponge has no asynchronous chunk API and no way to read a chunk that is not loaded, so Odyssey
 * force-loads chunks with a loading ticket and releases it as soon as the chunk has been copied.
 * That mechanism has a cost Paper's does not, which is why it has a budget setting Paper has no use
 * for.
 */
final class SpongeConfigKeys {

  /**
   * The most chunk-loading tickets Odyssey may hold at once. Mutable — the budget is consulted per
   * request.
   */
  ...
  SpongeConfigKeys(ConfigManager manager) {
    this.chunksMaxLoadRequests =
        manager
            ...
            .comment(
                """
                The most chunks Odyssey may hold loading tickets for at once. Odyssey needs a chunk
  ...
  static ConfigPlatform platform() {
    return new ConfigPlatform(
        ...
        Odyssey force-loads the chunk with a loading ticket, copies it, and releases the ticket
        straight away. See max_load_requests for how much of that may be in flight at once.
          ...
          allow_load
            Also load chunks that have already been generated. Odyssey checks that a chunk exists
