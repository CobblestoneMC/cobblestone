 ...
 * plugin classpath, so they do not have to be shaded into the jar. Paper downloads them via its
 ...
 * later pass can read the backend from config here and add only the one in use (design/06 notes
 * only the configured backend's driver need be present). bStats and Prometheus land with their
 * subsystems.
 */
