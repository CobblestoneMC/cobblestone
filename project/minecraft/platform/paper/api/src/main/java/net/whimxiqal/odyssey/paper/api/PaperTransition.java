 ...
 * return these from {@link PaperOdysseySearchModifier#computeTransitions}, so they never have to spell out
 * (or echo) the full generic signature. Build one with {@link #of}.
 */
public interface PaperTransition extends PlatformTransition<WorldRegion<World, Vector3i>, Location> {
  ...
  /**
   * A "wormhole" transition usable from anywhere in the player's current world: reach anywhere in that
   * world, run {@code command}, arrive at {@code destination}. The origin is the whole current world,
   * so the search can offer it immediately and prompt the command right away (e.g. {@code /home},
   * {@code /town spawn X}). This is the common shape for command-driven teleports.
   *
  ...
  static PaperTransition command(Player player, Location destination, double cost, String command) {
    return of(WholeWorldRegion.of(player.getWorld()), destination, cost,
