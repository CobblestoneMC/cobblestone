 ...
 *       (command warps, teleports, minecart lines) as {@link PaperTransition}s.
 ...
 * safe. The checker objects they return are invoked repeatedly <i>during</i> the search, possibly off
...
public interface OdysseySearchModifier {
  ...
  /**
   * Computes the mining-breakability check for the given player's search. Defaults to permitting all
   * breaking.
   *
   * @param player the player about to be routed
   * @return the break checker to apply for this search
   */
  default PaperBreakChecker computeBreakChecker(Player player) {
    return PaperBreakChecker.ALLOW;
  }
   ...
   */
  default PaperPassChecker computePassChecker(Player player) {
    return PaperPassChecker.ALLOW;
  }
}
