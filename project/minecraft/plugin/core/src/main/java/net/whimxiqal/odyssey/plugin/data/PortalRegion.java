...
/**
 * An axis-aligned block-coordinate box in one world, inclusive of both corners — used both for a
 * whole portal (the cache) and for a sub-region of one (a {@link PortalLink} entry cell group). A
 * portal's <b>anchor</b> is {@code (world, minX, minY, minZ)}: stable identity for the cache and
 * for links that reference it.
 *
...
public record PortalRegion(
  ...
  /** The world-space x of the region's horizontal centre (block centres). */
  public double centerX() {
    return (minX + maxX) / 2.0 + 0.5;
  }

  /** The world-space z of the region's horizontal centre (block centres). */
  ...
  /** Horizontal distance from a world-space point to this region's centre. */
