 ...
 * {@link #BASE}: {@code essentials.home.base} → {@code odyssey.navigate.essentials.home.base}. So
 * an admin can let players route to a place they cannot teleport to, or hide one they can.
 *
 * <p>Semantics are <b>default-allow</b>: navigation is permitted unless the node is explicitly set
 * to {@code false} (e.g. {@code -odyssey.navigate.towny.town.spawnhaven}, or a wildcard {@code
 * -odyssey.navigate.towny.*} on permission plugins that resolve wildcards). Only the full address
 * node is checked, so a permission plugin's specificity rules apply (a broad deny plus a specific
 * grant still grants).
 */
