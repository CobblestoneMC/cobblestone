...
/**
 * Odyssey's registered configuration parameters, grouped to mirror the generated {@code
 * config.yml}.
 *
...
public final class ConfigKeys {
  ...
  /**
   * How far Odyssey may go to obtain a chunk a search wants to walk through. The accepted values
   * and the default are platform-specific; see {@link ConfigPlatform}. Requires a restart — the
   * policy is captured in the chunk provider's settings when the platform API is built.
   */
  ...
  public ConfigKeys(ConfigManager manager, ConfigPlatform platform) {
    manager.header(
        ...
        ============================================================================================
         ...
         config copied from another platform may name settings this one does not support; Odyssey
         warns about those on startup and keeps its own defaults.

         Reloading: run "/odyssey reload" to re-read this file. Most values apply immediately.
    ...
    manager.section(
        "logging",
        """
        Logging verbosity. trace/debug surface Odyssey's own diagnostics (including a per-search
        summary at debug) on the console.""");
    this.loggingLevel =
        manager
            .key("logging.level", LogLevel.INFO, Codec.ofEnum(LogLevel.class))
            .comment("The lowest level of message Odyssey writes to the console.")
            .permitted(List.of(LogLevel.values()))
            .mutable()
            .register();

    manager.section("messages", "How Odyssey's chat messages look.");
    this.messagesShowPrefix =
        manager
            .key("messages.show_prefix", true, Codec.ofBoolean())
            .comment(
                "Whether to prepend the \"[✦]\" badge to every message Odyssey sends to a player.")
            .mutable()
            .register();
    ...
    this.dataFile =
        manager
            .key("data.file", "odyssey", Codec.ofString())
            .comment(
                """
    ...
    manager.section(
        ...
        much work Odyssey may ask the server to do on its behalf.""");
    this.chunksPolicy =
        manager
            .key(
    ...
    manager.section(
        "portals",
        """
        Vanilla portal links. No API reveals where a portal leads, so Odyssey learns them by
        watching players travel; use "/odyssey portals clear" to wipe what it has learned.""");
    ...
    this.portalsNormalizeEntry =
        manager
            ...
            .comment(
                ...
                (vanilla picks the destination from your exact sub-block position, which Odyssey
                cannot route to precisely). On by default: Odyssey's single source -> destination
