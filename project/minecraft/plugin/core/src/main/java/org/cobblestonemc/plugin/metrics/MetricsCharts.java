...
package org.cobblestonemc.plugin.metrics;
...
/**
 * Cobblestone's bStats chart set, defined once and shared across platforms. Each platform
 * constructs its own platform-specific {@code Metrics} (bstats-bukkit / bstats-sponge) and passes
 * that object's {@code addCustomChart} in here, so the chart ids, plugin id, and which gauge feeds
 * which chart all live in one place. The {@code org.bstats.charts.*} types are platform-neutral
 * (bstats-base).
 *
...
public final class MetricsCharts {
  ...
  /**
   * Adds Cobblestone's charts to a platform's metrics object.
   *
