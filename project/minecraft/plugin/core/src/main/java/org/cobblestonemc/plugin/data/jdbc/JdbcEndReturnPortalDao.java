...
final class JdbcEndReturnPortalDao implements EndReturnPortalDao {
  ...
  private static final String INSERT =
      "INSERT INTO cobblestone_end_return_portal (" + COLUMNS + ") VALUES (?,?,?,?,?,?,?,?)";
  private static final String SELECT_ALL =
      "SELECT " + COLUMNS + " FROM cobblestone_end_return_portal";
  private static final String EXISTS =
      "SELECT COUNT(*) FROM cobblestone_end_return_portal"
          + " WHERE world = ? AND min_x = ? AND min_y = ? AND min_z = ?";
  private static final String UPDATE =
      "UPDATE cobblestone_end_return_portal SET max_x = ?, max_y = ?, max_z = ?, cost = ?"
          + " WHERE world = ? AND min_x = ? AND min_y = ? AND min_z = ?";
    ...
    return store.inTransaction(
        ...
        connection -> {
          try (PreparedStatement delete =
              connection.prepareStatement("DELETE FROM cobblestone_end_return_portal")) {
