...
final class JdbcGatewayDao implements GatewayDao {
  ...
  private static final String INSERT =
      "INSERT INTO cobblestone_end_gateway (" + COLUMNS + ") VALUES (?,?,?,?,?,?,?,?,?)";
  private static final String UPDATE_EXIT =
      "UPDATE cobblestone_end_gateway SET to_world = ?, to_x = ?, to_y = ?, to_z = ?, cost = ?"
          + " WHERE world = ? AND x = ? AND y = ? AND z = ?";
  private static final String EXISTS =
      "SELECT COUNT(*) FROM cobblestone_end_gateway WHERE world = ? AND x = ? AND y = ? AND z = ?";
        ...
        connection -> {
          try (PreparedStatement select =
                  connection.prepareStatement(
                      "SELECT " + COLUMNS + " FROM cobblestone_end_gateway");
    ...
    return store.inTransaction(
        ...
        connection -> {
          try (PreparedStatement delete =
              connection.prepareStatement("DELETE FROM cobblestone_end_gateway")) {
