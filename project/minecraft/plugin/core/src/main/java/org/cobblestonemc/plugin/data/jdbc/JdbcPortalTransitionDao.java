...
final class JdbcPortalTransitionDao implements PortalTransitionDao {
  ...
  private static final String INSERT =
      "INSERT INTO cobblestone_portal_transition ("
          + COLUMNS
          + ") VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
  private static final String SELECT_ALL =
      "SELECT " + COLUMNS + " FROM cobblestone_portal_transition";
  private static final String EXISTS =
      "SELECT COUNT(*) FROM cobblestone_portal_transition"
          + " WHERE from_world = ? AND min_x = ? AND min_y = ? AND min_z = ?";
  private static final String UPDATE =
      "UPDATE cobblestone_portal_transition SET max_x = ?, max_y = ?, max_z = ?, to_world = ?,"
          + " to_x = ?, to_y = ?, to_z = ?, cost = ?"
          + " WHERE from_world = ? AND min_x = ? AND min_y = ? AND min_z = ?";
    ...
    return store.inTransaction(
        ...
        connection -> {
          try (PreparedStatement delete =
              connection.prepareStatement("DELETE FROM cobblestone_portal_transition")) {
