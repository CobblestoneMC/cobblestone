 ...
 * — a separate concern from whether they may <i>teleport</i> there (that stays the target plugin's
 * own permission). Every destination's address in the {@code /navigate} tree maps to a node under
 * {@link #BASE}: {@code essentials.home.base} → {@code cobblestone.navigate.essentials.home.base}.
 * So an admin can let players route to a place they cannot teleport to, or hide one they can.
 *
 * <p>Semantics are <b>default-allow</b>: navigation is permitted unless the node is explicitly set
 * to {@code false} (e.g. {@code -cobblestone.navigate.towny.town.spawnhaven}, or a wildcard {@code
 * -cobblestone.navigate.towny.*} on permission plugins that resolve wildcards). Only the full
...
public final class NavigationPermissions {
  ...
  public static boolean allowed(
      List<String> address, Predicate<String> isSet, Predicate<String> has) {
