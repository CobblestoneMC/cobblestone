 ...
 * CobblestoneColors#SECONDARY secondary} color; the surrounding literal text takes the message's
 * {@link MessageCategory} base color. Every line is prefixed with Cobblestone's {@code [✦]} badge
 * unless the prefix is disabled in config.
 *
...
public final class Messages {
  ...
  private static final String BUNDLE_BASE = "org.cobblestonemc.plugin.messages";
  private static final Object[] NO_ARGS = new Object[0];

  private static final Component PREFIX =
      Component.text()
          .append(Component.text("[", CobblestoneColors.PREFIX_FRAME))
          .append(Component.text("✦", CobblestoneColors.PRIMARY))
          .append(Component.text("] ", CobblestoneColors.PREFIX_FRAME))
          .build();
   ...
   */
  public Messages(Locale defaultLocale, boolean showPrefix, CobblestoneLogger logger) {
  ...
  private static Component param(Object arg) {
    if (arg instanceof Component component) {
      ...
      // own.
      return Component.empty().color(CobblestoneColors.SECONDARY).append(component);
    }
    return Component.text(String.valueOf(arg), CobblestoneColors.SECONDARY);
  }
